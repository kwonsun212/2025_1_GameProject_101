...
public class Monster: MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
              CharacterHit(AttackPoint);          //체력 포인트를 공격 포인트 만큼 감소 시켜준다.
        }
    ...
    {
     Timer -= Time.deltaTime;                //시간을 매 프레임마다 감소 시킨다.(deltaTime 프레임간의 시간간격을 의미한다.)

            if(Timer <= 0)                          //만약 Timer의 수치가 0이하로 내려갈 경우
        {
             Timer = 1.0f;                       //다시 1초로 변경 시켜준다.
             Health += 20;                       //1초마다 체력을 20을 올려주다.    
        }
    }
    ...
    {
        if (Health <= 0)                        //체력이 0이하로 내려가면 파괴한다.
